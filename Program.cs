...
class Program
{
    static async Task Main(string[] args)
    {
        ...
        foreach (var file in sourceFiles)
        {
            ...
            var root = await tree.GetRootAsync(); // Root is already available if needed, or re-get
            ...
            {
                // It's better to run Generate synchronously here or manage tasks carefully
                // if SemanticModel/Compilation objects are involved heavily in tasks.
                // For simplicity, let's make it direct for now.
                var tsContent = TypeScriptGenerator.Generate(node, tree, semanticModel, relativePath);
                var targetPath = Path.Combine(outputRoot, Path.ChangeExtension(relativePath, ".ts"));
                var targetDir = Path.GetDirectoryName(targetPath)!;
                Directory.CreateDirectory(targetDir);
                await File.WriteAllTextAsync(targetPath, "// This file has been generated by Cs2Ts\n// Do not edit as it will be overwritten\n"+tsContent);
